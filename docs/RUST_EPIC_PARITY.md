...
| one chunk exhausting all CDNs → `failCount++`, pool continues, install fails at the end | `:416-421`, `:479-483` | DEV — the core aborts the run when an item exhausts its attempts; other in-flight chunks finish and are published (verified), queued ones are not fetched. Same terminal outcome (`ERROR: 1 chunks failed` → `false`, `markFailed("Download failed")`), less wasted transfer |
